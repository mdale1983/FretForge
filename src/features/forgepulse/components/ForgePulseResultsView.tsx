...
export function ForgePulseResultsView({ theme, timing, onAgain, onSetup }: { theme: string; timing: TimingCoachState; onAgain: () => void; onSetup: () => void }) {
  ...
  return <div className={`mt-5 rounded-xl border p-5 ${theme === "dark" ? "border-zinc-800 bg-zinc-950/50" : "border-zinc-200 bg-zinc-50"}`}>
    ...
    <div className="mt-5 flex flex-wrap gap-3"><button type="button" onClick={onAgain} className="rounded-lg bg-orange-500 px-5 py-2 text-sm font-semibold text-white">Practice Again</button><button type="button" onClick={onSetup} className="ml-auto rounded-lg border border-zinc-700 px-4 py-2 text-sm font-semibold">Back to Setup</button></div>
  </div>;
}

function Result({ label, value }: { label: string; value: string }) { return <div className="rounded-lg border border-zinc-700/60 p-3"><p className="text-[11px] uppercase tracking-wide text-zinc-500">{label}</p><p className="mt-1 font-semibold tabular-nums">{value}</p></div>; }
